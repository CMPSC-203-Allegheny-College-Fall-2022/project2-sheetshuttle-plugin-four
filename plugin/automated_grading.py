...
STANDARD_REPO_NAME = "CMPSC-203-Allegheny-College-Fall-2022/gradebook"
...
CONFIG_WRITE_FILENAME = "ss_automated_gh_config.yml"
...
def run(sheets_keys_file, sheets_config_directory, gh_config_directory, **kwargs):
    ...
    practical_grades = used_config.regions["Sheet1_practicals"].data
    projects_grades = used_config.regions["Sheet1_projects"].data
    quiz_grades = used_config.regions["Sheet1_quizzes"].data
    ...
    for student_index in range(0, row_count):
        ...
        # iterate through EE
        start_index = 0
        while start_index < len(practical_grades.iloc[student_index]):
            current_student.add_practical(
                practical_grades.iloc[student_index][start_index : start_index + 4]
            )
            start_index += 4
        ...
        start_index = 0
        while start_index < len(quiz_grades.iloc[student_index]):
            current_student.add_survey(quiz_grades.iloc[student_index][start_index])
            start_index += 1
...
class Student:
    ...
    def add_practical(self, grade_list):
        # TODO: account for Nan Values
        crit_prcnt = ((grade_list[1] + grade_list[2] + grade_list[3]) / 3) * 100
        practical_prcnt = (grade_list[0] + crit_prcnt) / 200
        self.grades["practicals"]["sum"] += practical_prcnt
        self.grades["practicals"]["num"] += 1
    ...
    def add_quiz(self, grade):
        # TODO: account for Nan Values
        self.grades["quizzes"]["sum"] += grade
        self.grades["quizzes"]["num"] += 1
    ...
    def generate_new_issue_config(self):
        config = {
            "type": "issue",
            "action": "create",
            "repo": f"{STANDARD_REPO_NAME}-{self.username}",
